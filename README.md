...
By default `run` writes `reports/audit-<timestamp>.md` (and a matching `.pdf` if Pandoc + a PDF engine are available) instead of printing to stdout — `--output <path>` overrides where the Markdown goes. A live progress UI (readiness message per category, each check printed as it runs with its actual findings listed underneath — severity + title, color-coded by severity — an `<alias> completed` line once each category finishes, errors additionally logged to `logs/audit-<timestamp>.log`) is on by default when stderr is a terminal, off otherwise (redirected output, cron, CI) — override either way with `--progress`/`--no-progress`. All of this status output goes to **stderr**; the report itself only ever goes to the file (or, historically, stdout) — `... > /dev/null` never clips the report.

A check only shows a green ✓ **passed** when it turns up zero findings — a check that runs cleanly but surfaces even one low-severity finding shows a red ✗ **completed** instead, since "passed" would gloss over something that still needs addressing. The checkmark/X only says whether anything needs attention at all; the severity color-coding on each finding line underneath (red/yellow/cyan/dim for High-through-Informational) says how urgent it is.

`--step` (on by default for `ia run`, off otherwise) pauses after each category's "Ready to run..." message and waits for you to press Enter before it actually runs that category — otherwise a full 11-category audit just blasts through unattended, which for a client-facing run makes it too easy to miss what's flying by. Pass `--no-step` to run uninterrupted.
...
A full run against it currently surfaces genuine findings in every automated category (1–11) — confirmed via manual CLI runs, not just design intent. Two gotchas it exists to document:
- The seeded `sessions` table needs its dead-tuple bloat (07.04/10.03) to survive until an audit actually runs, but Postgres's own autovacuum daemon was cleaning it up within its default ~1-minute naptime before a manual audit got around to it, making those two findings flaky. Fixed by starting the container with `-c autovacuum=off` cluster-wide — deliberately not a per-table `reloptions` override, since 07.04 specifically requires `reloptions IS NULL` to fire (it's checking for bloat on cluster-wide defaults, not a tuned-and-still-struggling table).
- The DSN uses `127.0.0.1`, not `localhost`. Docker's port binding (`-p 127.0.0.1:55432:5432`) is IPv4-only, but `localhost` frequently resolves to IPv6 (`::1`) first on Windows — psycopg would then hang on a dead IPv6 connection attempt until it timed out and fell back to IPv4, turning every single check into what looked exactly like a hung/frozen CLI. Measured directly: the identical check took 2m10s with `localhost` in the DSN vs. 0.7s with `127.0.0.1` — essentially the entire delay was TCP timeout wait (confirmed via `user`/`sys` CPU time being near zero across the full 2+ minutes), not real work.
...
- **Core** (`src/integri_audit_tool/`) — CLI entrypoint (`cli.py`), read-only DB connection handling (`db.py`), the `Finding`/`Severity`/`AuditReport` data model (`models.py`), dynamic category discovery (`registry.py`), audit orchestration (`runner.py`), and the Markdown report renderer (`report/markdown.py`).
  - The Executive Summary and Findings Summary Table were both flagged as unreadable in the actual generated PDF, not just in the raw Markdown: the summary was a bracket-tagged list (`[High] ...`) with no connecting prose, and the table's rightmost column held full-sentence business-impact text that rendered 15-20 characters wide. Two separate fixes: (1) the executive summary now opens with real sentences (finding counts + an urgency framing sentence) and pairs each top finding with *why it matters*, reusing the already-well-written `business_impact` text rather than inventing new content — kept fully deterministic (no LLM call in the pipeline; every other piece of report text in this tool is hand-authored ahead of time, and a client-facing audit deliverable is exactly the wrong place to introduce non-determinism or hallucination risk). (2) The table's Business Impact column was dropped entirely (the same text already lives in Detailed Findings, more readably) and Category was replaced with the compact, more precise `check_id`. That alone wasn't enough, though — Pandoc's pipe-table→LaTeX conversion sizes each column *relative to the dash-count in its separator row*, not to actual content length, so an equal-width separator (`|---|---|---|---|`) produces four equal-width PDF columns regardless of what's actually in them (documented Pandoc behavior, confirmed empirically, not a workaround). `_TABLE_COL_WIDTHS` now builds a proportional separator row, giving the prose-holding Finding column the bulk of the space — confirmed via a real generated PDF: 26 findings that previously sprawled across 3 cramped pages now fit cleanly on one.
...
- **Live progress UI + per-check CLI selection + PDF export** are a deliberate case study in keeping a feature "not core": `reporter.py` defines an `AuditReporter` Protocol (category/check start/success/failure/complete hooks, plus `category_completed` — fired once per category regardless of outcome, the one hook every reporter below actually needs) and a `NullReporter` no-op default — `runner.py` calls these hooks but never imports anything presentation-related, so every existing test that calls `run_audit()` without a `reporter` argument is untouched. The actual colors/progress-bars/error-logging implementation (`cli_progress_reporter.py`, using `rich`) and PDF conversion (`pdf_export.py`, shelling out to `pandoc` — a system binary, not a Python dependency, since it has no PDF renderer of its own without a separately-installed engine) live entirely outside core; swapping in a different `AuditReporter` (a JSON-lines one for CI, say) never touches `runner.py`. `AuditConfig.check_filter` extends the existing `category_filter` pattern one level down, to individual check ids (`-k 01.04`), backing `scripts/run-check.sh`/`run-category.sh` — thin Bash wrappers, since that's the shell the tool is meant to be driven from day to day.
  ...
  - The per-category progress bars (`rich.progress.Progress`) were later removed entirely, also found via live testing: `Progress` renders as a persistent, self-redrawing region that interleaves unpredictably with the plain `console.print()` calls used for readiness/check/finding lines — in practice category 1's completed bar rendered *after* category 2's readiness message had already printed. Since every check and its findings are already printed line by line, the bar was redundant on top of being visually broken; `CliProgressReporter` now tracks "was this category announced" with a plain `set[int]` instead of a `dict[int, TaskID]`, and there's no live-rendering region left to race against sequential prints.
