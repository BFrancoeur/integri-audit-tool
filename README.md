...
- Implemented so far: **Schema Design & Normalization Boundaries** (category 1, 3 of 6 bullets), **JSONB Structure & Governance** (category 2, 3 of 6 bullets), **Indexing Strategy** (category 3, 3 of 7 bullets), and **Full-Text & Structured Search Behavior** (category 4, 2 of 6 bullets). The remaining 7 categories exist as stub packages (`CATEGORY` with an empty `checks` list) ready to be filled in the same way, incrementally, working down the rubric in order.
- Category 4 is the thinnest so far on purpose: 4 of its 6 bullets (relevance ranking, safe query parsing, combining structured+free-text filters, facets matching governed attributes) live in application/query code that a read-only DB connection can't see at all — only "is full-text search indexed with GIN/GiST" and "is the tsvector kept in sync via a generated column or trigger" are genuinely catalog-checkable. Better to implement fewer checks honestly than stretch DB introspection to fake-cover bullets it can't actually assess.
